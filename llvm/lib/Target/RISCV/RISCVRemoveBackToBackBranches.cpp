...
//===----------------------------------------------------------------------===//
//
// This pass prevents back-to-back conditional branches from appearing in the
// same 8-byte-aligned region of code on the RISC-V MIPS p8700 CPU. By inserting
// alignment directives where necessary, we ensure that each conditional branch
// resides in a distinct region. This mitigates hardware hazards on the p8700
// and helps improve overall performance.
//
//===----------------------------------------------------------------------===//
...
STATISTIC(NumInsertedAligments, "Number of alignments set");
...
bool RISCVRemoveBackToBackBranches::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    ...
    for (auto &Successor : MBB.successors()) {
      // Do not align if the successor is the same block.
      if (&MBB == Successor)
        continue;
      ...
      // NOTE: Alignment won't fix hazards if the successor is placed before the
      // branch in final layout.
